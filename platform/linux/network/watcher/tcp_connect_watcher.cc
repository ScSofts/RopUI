...
class EpollTcpConnectWatcher final : public ITcpConnectWatcher {
public:
  EpollTcpConnectWatcher(IOWorker &worker, IpEndpoint remote,
  ...
  ~EpollTcpConnectWatcher() override {
    stop();
    source_.reset();
    TcpDetail::closeFd(fd_); // when attached_ = 0
  }
  ...
  void start() override {
    ...
    source_ = std::make_shared<RopHive::Linux::EpollReadinessEventSource>(
        fd_, EPOLLOUT | EPOLLERR | EPOLLHUP,
...
private:
  ...
  void checkConnectResult() {
    ...
    auto stream =
        std::make_unique<LinuxTcpStream>(TcpStreamKind::LinuxEpoll, fd_);
    fd_ = -1;
    stop();
...
private:
  ...
  void checkConnectResult() {
    ...
    auto stream =
        std::make_unique<LinuxTcpStream>(TcpStreamKind::LinuxPoll, fd_);
    fd_ = -1;
    stop();
    if (option_.fill_endpoints) {
      TcpDetail::bestEffortFillEndpoints(stream->fd(), *stream);
...
createEpollTcpConnectWatcher(IOWorker &worker, IpEndpoint remote,
                             TcpConnectOption option,
                             ITcpConnectWatcher::OnConnected on_connected,
                             ITcpConnectWatcher::OnError on_error) {
  return std::make_shared<EpollTcpConnectWatcher>(
