#ifndef _ROP_PLATFORM_MACOS_NETWORK_WATCHER_TCP_SOCKET_COMMON_H
#define _ROP_PLATFORM_MACOS_NETWORK_WATCHER_TCP_SOCKET_COMMON_H
...
namespace RopHive::MacOS::TcpDetail {
...
inline void applyNoSigPipeIfAvailable(int fd) {
#ifdef SO_NOSIGPIPE
  const int yes = 1;
...
inline void bestEffortFillEndpoints(int fd,
  ...
  if (!stream.local.has_value() &&
      ::getsockname(fd, reinterpret_cast<sockaddr *>(&local), &local_len) ==
          0) {
    stream.local = fromSockaddr(reinterpret_cast<sockaddr *>(&local), local_len);
  }
}
...
class MacTcpStream final : public RopHive::Network::ITcpStream {
public:
  MacTcpStream(RopHive::Network::TcpStreamKind kind, int fd)
      : ITcpStream(kind), fd_(fd) {}

  ~MacTcpStream() override { TcpDetail::closeFd(fd_); }
...
#endif // _ROP_PLATFORM_MACOS_NETWORK_WATCHER_TCP_SOCKET_COMMON_H
