...
public:
    ...
    void join(std::shared_ptr<Client> client) { cmds_.push(RoomCommand{RoomCommand::Kind::Join, std::move(client)}); }
...
private:
    void handle(const RoomCommand& cmd) {
        ...
        case RoomCommand::Kind::Join: {
            ...
            broadcastImpl("[server] user#" + std::to_string(cmd.client->id) + " joined (" +
...
static asyncnet::Task<void> chatSession(asyncnet::Executor& exec,
    ...
    for (;;) {
        ...
        for (auto line : splitLines(buffer)) {
            ...
            std::string msg = "user#" + std::to_string(client->id) + ": " +
                              line + "\n";
